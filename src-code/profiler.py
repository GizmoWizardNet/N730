...
Outputs a sensitivity report + the quantization map used by the N730 format converter.
...
def assign_precision(sensitivity: float) -> str:
    """
    Maps sensitivity score to a quantization level.
    Thresholds tuned for GT 730's 2GB budget:
      - Be aggressive (INT2) on safe middle layers
      - Protect (INT8) on critical layers
    """
    if sensitivity >= 0.75:
        return "INT8"
...
def profile_weight_tensor(
    ...
    precision = assign_precision(sensitivity)
...
def main():
    parser = argparse.ArgumentParser(
        description="N730 Precision Profiler — Project Bombakla Phase 1"
    )
    ...
    if args.synthetic:
        print(f"  Mode: SYNTHETIC  ({args.layers} layers, hidden={args.hidden})")
        model_id = f"synthetic-{args.layers}L-h{args.hidden}"
        layers = generate_synthetic_layers(args.layers, args.hidden)
    elif args.model:
        print(f"  Mode: REAL MODEL  ({args.model})")
        model_id = args.model
        layers = load_model_layers(args.model)
    else:
        print("  No model specified. Running in synthetic mode (--layers 32).")
